...
class SweetAlertNotifier
{
    ...
    /**
     * @var \SweetAlert2\SweetAlert\SessionStore
     */
    protected $session;
     ...
     *
     * @param \SweetAlert2\SweetAlert\SessionStore $session
     */
    ...
    {
        $this->config = [
            'timer'   => config('sweetalert2.autoclose', self::TIMER_MILLISECONDS),
            'text'    => '',
            'buttons' => [
                'cancel'  => false,
                'confirm' => false,
     ...
     * @param string $icon
     *
     * @return \SweetAlert2\SweetAlert\SweetAlertNotifier $this
     */
    ...
    protected function flashConfig()
    {
        foreach ($this->config as $key => $value) {
            $this->session->flash("sweetalert2.{$key}", $value);
        }
